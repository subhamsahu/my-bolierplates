exports.resetEmail = (host, data) => {
  const message = {
    ...
    `http://${host}/auth/reset/password/${data.resetToken}`
  };
...
exports.signupEmail = (data) => {
  const message = {
    subject: 'Account Registration',
    text: `Hi ${data.registeredUser.firstName} ${data.registeredUser.lastName}! Thank you for creating an account with us!.
           Email Verification Link: http://127.0.0.1:6080/api/auth/email/verify/?token=${data.token}`
  };
